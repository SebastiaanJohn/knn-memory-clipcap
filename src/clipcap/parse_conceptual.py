...
def thread(
    ...
    out_data_path = (
        f"{conceptual_root}/conceptual_{suffix}_{thread_id:02d}.pkl"
    )
    recover_index = 0
    if os.path.isfile(out_data_path):
        with open(out_data_path, "rb") as f:
    ...
    for i in range(0, len(urls)):
        ...
        if (i + 1) % 1000 == 0:
            save_pickle(
                {"parsed": parsed, "info": info}, out_data_path, recover_index
            )
...
def create_clip_embeddings(conceptual_root: str, clip_model_type: str):
    ...
    for suffix in ("val", "train"):
        ...
        for i, data in enumerate(dl):
            ...
            captions = [
                caption for j, caption in enumerate(captions) if is_valid[j]
            ]
            image_names = [
                image_name
